...
describe('Reports Model', () => {
  ...
  describe('createOrReplace', () => {
    it('should create new report file when no existing file found', async () => {
      ...
      expect(require('../../middleware/storage').upload).toHaveBeenCalledWith(mockPath, mockFile, bucket);
    ...
    it('should replace existing report file when file exists', async () => {
      ...
      const bucket = 'reports';
      const oldPath = '1/Midterm_1234567890';
      const newPath = '1/Midterm_1234567891';
      const newUrl = 'https://example.com/reports/1/Midterm_1234567891';
      ...
      expect(require('../../middleware/storage').upload).toHaveBeenCalledWith(newPath, mockFile, bucket);
  ...
  describe('delete', () => {
    ...
    it('should handle missing file path gracefully', async () => {
      ...
      await reportsModel.delete(mockFile, bucket);
  ...
  describe('Integration Tests', () => {
    it('should handle complete report file lifecycle', async () => {
      const mockData = {
        gradeid: '1',
        studentid: '1',
        test_type: 'Midterm'
      ...
      // Mock successful operations for create
      require('../../middleware/storage').upload.mockResolvedValue();
      require('../../middleware/storage').getPublicUrl.mockResolvedValue(mockUrl);
      ...
      const createResult = await reportsModel.create(mockData, mockFile, bucket);
      expect(createResult.path).toBe(mockPath);
      expect(createResult.url).toBe(mockUrl);

      // 2. Replace report file
      const newFile = {
        buffer: Buffer.from('updated report content'),
        mimetype: 'application/pdf'
      };
      ...
      // Mock storage operations for replacement
      require('../../middleware/storage').delete.mockResolvedValue();
      require('../../middleware/storage').upload.mockResolvedValue();
      require('../../middleware/storage').getPublicUrl.mockResolvedValue(newUrl);
      ...
      const replaceResult = await reportsModel.createOrReplace(mockData, newFile, bucket);

      expect(require('../../middleware/storage').delete).toHaveBeenCalledWith(mockPath, bucket);
      expect(require('../../middleware/storage').upload).toHaveBeenCalledWith(newPath, newFile, bucket);
      expect(replaceResult.path).toBe(newPath);
      expect(replaceResult.url).toBe(newUrl);
      ...
      await reportsModel.delete(replaceResult, bucket);
      expect(mockSupabaseStorage.remove).toHaveBeenCalledWith(newPath);
    });
  });
