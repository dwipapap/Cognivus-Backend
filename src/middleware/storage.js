...
exports.upload = async (path, file, bucket = DEFAULT_BUCKET) => {
  ...
  const payload = file && file.buffer ? file.buffer : file;
  const options = file && file.mimetype ? { contentType: file.mimetype, upsert: true } : { upsert: true };
...
exports.delete = async (pathOrPaths, bucket = DEFAULT_BUCKET) => {
  ...
  const arg = isArray ? pathOrPaths : pathOrPaths; // keep original value
